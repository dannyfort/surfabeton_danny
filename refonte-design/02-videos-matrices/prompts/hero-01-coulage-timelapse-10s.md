# HERO 01 — « Le Coulage » (vidéo matrice du site) — SEEDANCE 2.0 · 10 s

Version **10 secondes** de la matrice (la version 15 s est dans
`hero-01-coulage-timelapse.md`). Contenu identique, re-timé serré.
...
| Paramètre | Valeur |
|---|---|
| Durée | **10 s** (prompt découpé sur 10 s — sélectionner 10 s à la génération) |
...
| `@Image3` | **plan large hangar + pro sur truelle mécanique** (l'image fournie) | **LAST FRAME exacte** | t = 10 s |

> On ancre **première ET dernière frame** — le levier Seedance le plus fiable pour
> finir précisément sur une image donnée.
...
Photorealistic cinematic film, 10 seconds, 16:9. ONE continuous crane-mounted
...
2–5s: The camera keeps the macro locked and static. From the left edge of frame, a
...
accelerated trowel passes sweep through, and the surface smooths, cures and hardens
within seconds into a flawless mirror-polished concrete floor — sheen, texture and
warm window-light reflections exactly matching @Image2.

5–6s: A single beat on the mirror finish, then an aggressive fast dolly-in punches
toward the glowing liquid-gold reflection — a snap push with FPV energy and heavy
motion blur.

6–7s: Without any cut, the camera whips straight out of that punch-in into a
violent, ultra-fast FPV pull-back — drone-velocity rocketing backward and upward,
extreme motion blur, the whole hall smearing past in streaks of light. Aggressive,
kinetic, crash-dezoom energy, one unbroken high-speed move.

7–10s: The FPV pull-back rapidly decelerates and settles onto the wide establishing
shot of @Image3; the camera then locks off completely fixed and static, no further
movement, holding this final composition for the full last three seconds. Within
the locked frame, the professional in the hi-vis yellow jacket, seen from behind,
slowly rides the ride-on power trowel forward across the gleaming concrete, its
rotating blades smoothing and polishing the surface to an even sheen as he
navigates the floor, long light reflections stretching toward the camera. The final
frame must match @Image3 exactly in composition, framing and lighting.

Camera: one continuous crane/FPV-mounted shot, NO CUT — locked and static on the
macro for the first 5 seconds, then a very fast aggressive dolly-in punch, whipping
straight into an ultra-fast FPV pull-back with extreme motion blur, and finally
settling into a completely fixed, locked-off wide shot that holds static on @Image3
for the last three seconds while the machine moves slowly. Fast aggressive moves in
the middle, a calm static hold at the end — but a single unbroken take.
...
## Pourquoi ces choix (version 10 s)
...
- **Re-timing serré** : le couli + timelapse passe de 5 s à **3 s** (2–5 s), les
  moves rapides gardent leur punch. Sur 10 s c'est le segment « hyper-fast timelapse »
  qui absorbe la compression — le reste du rythme est préservé.
- **Le couli reste sur macro verrouillé** : lock 0–5 s, énergie cinétique 5–10 s.
- **Enchaînement cinétique 5–7 s, puis hold** — le cœur : *aggressive dolly-in*
  (punch FPV sur le reflet ciré, 5–6 s) → *FPV pull-back* whip motion blur extrême
  **ultra-court, 1 s** (6–7 s). Le punch-in bref **avant** le pull-back crée le
  « changement très rapide de caméra » (in → out) tout en restant un plan unique
  sans coupe. Le beat d'1 s (5–6 s) évite que punch-in et pull-back fusionnent.
- **Fin en caméra fixe, 7–10 s (3 s)** : après le crash FPV, la caméra se
...
- **`@Image2` comme cible du timelapse** (et non frame finale) : l'état ciré sert de
  référence de rendu au milieu du plan, la fin est réservée à `@Image3`.

### Vocabulaire caméra — curseurs d'intensité (à doser)

Du plus doux au plus violent — remplace/renforce selon le rendu voulu :
...
⚠️ Sur 10 s, l'espace est encore plus court : garder **un seul** gros mouvement +
le punch. Empiler trop d'accents rapides provoque une bouillie ou une coupe parasite.
